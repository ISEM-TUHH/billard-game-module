...
class MongoDB:
    ...
    def assert_aggregation(self, agg):
        # get a random document to test the aggregation on
        aggCursor = self.collection.aggregate([{"$sample": {"size": 1}}])
    ...
    def get_score(self, _id="latest", insert=True, new_agg=False):
        if _id == "latest":
        ...
        agg = new_agg if new_agg else self.agg
