...
class GameMode:
    ...
    def entrance(self, inp):
        ...
        if self.state == new_state and "gameimage-updates" in local_returns.keys():
    ...
    def history(self, history=None, add=None, get_semester=None):
        ...
        from_df = lambda x: [[v["score"], v["player"], v["team"], v["semester"], v["attestation"]] for i, v in x.iterrows()] # score, player, team  
    ...
    def build_HTML(self):
        ...
        for k, v in self.TREE.items():
            ...
            if button_name is not None:
                ...
                html = f"<input type='checkbox' id='{ID}'><label for='{ID}' data-og='{button_name}'>{button_name}</label>"
    ...
    def check_starting_positions(self, inp, starting_positions=[], tolerance=50):
